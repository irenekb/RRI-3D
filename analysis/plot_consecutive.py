...
clusters=10 #CLUSTER
exsteps=20 #ROUNDS
expansionsteps = list(range(3,6,2)) #start, end, interval
consecutive_run = "consecutive" #for outputname: consecutive, nonconsecutive, target
...
for step in stepsize1:
    ...
    for run in range(exsteps):
        ...
        group1 = new_df.groupby(["len_interaction"]).agg({'len_interaction': ['count'],
                                                            'energy_value': ['min', 'median', 'mean'],
                                                            'energy_diff':['min','max','median','mean']}
                                                    ).reset_index()
        group1.columns =['interaction_length', 'nr',
        ...
        group1.sort_values(by=['interaction_length'])
        ...
        group2 = new_df.groupby(["interaction_countbp"]).agg({'interaction_countbp': ['count'],
                                                            'energy_value': ['min', 'median', 'mean'],
                                                            'energy_diff':['min','max','median','mean']}
                                                         ).reset_index()
        group2.columns =['interaction_bp', 'nr',
                          'energy_min', 'energy_median','energy_mean',
                          'dif_min','dif_max', 'dif_median','dif_mean']
        group2['percent'] = (group2.nr /group2.nr.sum())*100
        group2['constraint'] = expansionsteps[run]
        group2.sort_values(by=['interaction_bp'])
    ...
    labely = heatmapdata1.interaction_length.unique().tolist()
    labely.sort()
    ...
    def get_labels(heatmap):
        ...
        ly = heatmap.interaction_length.unique().tolist()
        ly.sort()
    ...
    for step in stepsize1:
        ...
        for cl in range(clusters):
            ...
            for run in range(exsteps):
                ...
                group = new2_df.groupby(["len_interaction"]).agg({'len_interaction': ['count'],
                                                                    'energy_value': ['min', 'median', 'mean'],
                                                                    'energy_diff':['min','max','median','mean']}
                                                                 ).reset_index()
                group.columns =['interaction_length', 'nr',
                                  'energy_min', 'energy_median','energy_mean',
                                  'dif_min','dif_max', 'dif_median','dif_mean']
                group['percent'] = (group.nr /group.nr.sum())*100
                group['constraint'] = expansionsteps[run]
                group.sort_values(by=['interaction_length'])
            ...
            labely = heatmapdata3.interaction_length.unique().tolist()
            labely.sort()
